//! The `JsonUnion` sparse union used as the return type of `jmespath()`.
//!
//! Adapted from `datafusion-functions-json`, so that a JMESPath result keeps
//! its JSON type — a string, a number, a bool, or nested JSON — instead of
...
pub(crate) fn nested_json_array(array: &ArrayRef, object_lookup: bool) -> Option<&StringArray> {
    ...
    Some(union_array.child(type_id).as_string())
}
